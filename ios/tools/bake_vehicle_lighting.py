...
bpy.ops.mesh.primitive_plane_add(size=8,location=(0,0,-.003));ground=bpy.context.object;ground.data.materials.append(bpy.data.materials.new('Studio matte ground'));ground.data.materials[0].diffuse_color=(.008,.008,.008,1)
...
gi=np.empty(len(ca.data)*4,dtype=np.float32);ca.data.foreach_get('color',gi);gi=gi.reshape(-1,4);
...
bpy.ops.wm.save_as_mainfile(filepath=str(O/'RuntimeOcclusion.blend'),compress=True)
