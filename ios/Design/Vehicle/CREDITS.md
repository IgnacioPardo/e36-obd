...
The reference-model studio combines Pine Picnic with softened Studio Small 09 lighting. Its HD previews render reflections and shadows together in Cycles. The live app's default studio uses a rotated, commonly exposed Studio Small 09 photographic HDR; the optional daylight scene uses Park Parking with its matching ground-projected background. The studio has geometry-baked diffuse illumination and ambient visibility; Metal evaluates view-dependent specular, clearcoat and glass reflections. Daylight uses full live PBR and baked ambient visibility. Transparent ground-shadow textures contain no car pixels. The app does not use an offline car-image sequence.
